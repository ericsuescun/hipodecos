class PhysiciansController < ApplicationController
  ...
  def update
    ...
    if @physician.update(physician_params)
      ...
      if params[:physician][:inform_id] != nil
        inform = Inform.find(params[:physician][:inform_id])
        if inform.inf_status == "revision"
          redirect_to show_revision_inform_path(inform), notice: 'Informe exitosamente actualizado.'
        else
          redirect_to @physician.inform, notice: 'Médico exitosamente editado.'
        end
      else
        redirect_to @physician, notice: 'Médico exitosamente editado.'
      end
      ...
      #   objection.save
      # end
      # redirect_to inform_path(@inf), notice: 'Médico ha sido exitosamente actualizado.'
    else
      render :edit
    end
  end
