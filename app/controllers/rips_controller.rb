  ...
  def procedures
    ...
    csv_file = CSV.generate(headers: true) do |csv|
      ...
      Patient.joins(:informs).where({informs: { delivery_date: date_range, invoice: params[:invoice] }}).distinct.find_each do |patient|
        ...
        patient.informs.publ_down.each do |inform|
          inform.studies.each do |study|
            study.factor&.times do
              csv << [
                ...
                inform.related_diagnostic_code, "", study.price, inform.copayment.present? ? "01" : "05", inform.copayment, inform.invoice, patient_index_counter,
